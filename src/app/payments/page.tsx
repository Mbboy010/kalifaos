...
const PaymentPage = () => {
  ...
  const DetailCard = ({ label, value, id }: { label: string; value: string; id: string }) => (
    <div className="bg-[#1a1a1a] border border-zinc-800 p-4 rounded-xl flex justify-between items-center hover:border-zinc-600 transition-colors">
      <div>
        <p className="text-zinc-500 text-xs font-medium uppercase mb-1">{label}</p>
      ...
      >
        {copied === id ? <CheckCircle2 className="w-5 h-5 text-green-500" /> : <Copy className="w-5 h-5 text-zinc-400" />}
      </button>
    </div>
  );
    ...
    <div className="min-h-screen bg-black p-6 md:p-12">
      <div className="max-w-4xl mx-auto">
        <header className="mb-10 text-center">
          <h1 className="text-3xl font-extrabold text-white mb-2">Payment Methods</h1>
          <p className="text-zinc-400">Select a preferred method to complete your transaction</p>
        </header>

        {/* Method 1: Crypto */}
        <PaymentSection title="Crypto Wallets" icon={Wallet} color="text-yellow-500">
        ...
        {/* Method 2: Dollar */}
        <PaymentSection title="USD / International" icon={CreditCard} color="text-blue-500">
        ...
        {/* Method 3: Naira */}
        <PaymentSection title="Naira / Local Bank" icon={Landmark} color="text-green-500">
        ...
        <footer className="mt-12 p-6 border-t border-zinc-800 text-center">
