...
// Define the type for your Firestore documents
interface ToolData {
  id: string;
  title: string;
  version: string;
    ...
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg max-w-2xl mx-auto">
          Download specialized tools to bypass Factory Reset Protection (FRP) on Android devices.
          Use with caution and only on devices you legally own.
        </p>
      </div>

      {/* Security Warning */}
      <div style={{ backgroundColor: isColor ? "#4f16164c" : "#f0cece4c" }} className="rounded-xl border-l-4 border-red-500 p-4 mb-8">
        ...
        <div className="w-screen flex flex-col gap-2">
          {istrue ? (
            data.map((element: ToolData, index: number) => (
              <a className="px-4 py-1" href={element.link} key={index}>
                <div style={{ backgroundColor: isColor ? "#d7d7d719" : "#72727236" }} className="flex items-center p-3 shadow rounded-lg">
            ...
            <div className="w-screen flex flex-col gap-2">
              {[...Array(3)].map((_, idx) => (
                <a className="px-4 py-1" key={idx}>
                  <div style={{ backgroundColor: isColor ? "#d7d7d719" : "#72727236" }} className="flex items-center p-2 shadow rounded-lg">
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      <p className="text-gray-500 cousss h-2 w-[70%]" />
                    </div>
                  </div>
                </a>
              ))}
      ...
      {/* Disclaimer */}
      <div style={{ backgroundColor: isColor ? "#d7d7d719" : "#72727236" }} className="mt-12 p-4 rounded-lg text-center text-sm">
        <p>
          These tools are provided for educational purposes only. Use at your own risk.
          We do not condone illegal use of FRP bypass tools.
        </p>
      </div>
    </div>
  );
