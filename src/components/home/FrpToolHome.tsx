...
import { useAppSelector } from '@/components/redux/hooks'; // Adjust path if needed
...
export default function FrpToolHome() {
  ...
  return (
    <div className={`min-h-screen font-sans relative overflow-hidden flex flex-col items-center transition-colors duration-500 ${
      isColor ? 'bg-[#050505] text-slate-300 selection:bg-cyan-500/30' : 'bg-slate-50 text-slate-900 selection:bg-blue-500/30'
    }`}>
      
      {/* Background Decor */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className={`absolute top-0 right-1/4 w-[500px] h-[500px] rounded-full blur-[120px] transition-opacity duration-500 ${
          isColor ? 'bg-cyan-500/10 opacity-100' : 'bg-blue-500/10 opacity-60'
        }`}></div>
        <div className={`absolute bottom-0 left-1/4 w-[400px] h-[400px] rounded-full blur-[100px] transition-opacity duration-500 ${
          isColor ? 'bg-purple-500/10 opacity-100' : 'bg-indigo-500/10 opacity-60'
        }`}></div>
      </div>
      ...
      <div className="w-full max-w-3xl px-6 py-12 flex flex-col items-center z-10 flex-1">
        ...
        {/* Phone Mockup Hero */}
        <div className="relative w-[360px] h-[460px] flex justify-center mb-8 drop-shadow-2xl hover:scale-[1.02] transition-transform duration-700">
          
          {/* Green Power Button */}
          <div className="absolute right-[11px] top-[140px] w-[4px] h-[50px] bg-[#81c995] rounded-r-md shadow-sm z-0"></div>
          
          {/* Phone Body (Remains Dark for Hardware realism) */}
          <div className="relative w-[330px] h-full bg-[#111] rounded-t-[2.5rem] border-[4px] border-b-0 border-slate-800 overflow-hidden flex flex-col z-10 shadow-2xl">
            ...
            <div className="h-[72px] w-full shrink-0 relative flex items-center">
              ...
              {/* Speaker Grill */}
              <div className="absolute left-1/2 -translate-x-1/2 w-16 h-1.5 bg-[#1a1a1a] rounded-full shadow-inner"></div>
            </div>

            {/* Phone Screen */}
            <div className="flex-1 bg-white flex flex-col relative">
              {/* Google Header */}
              <div className="bg-[#5A7DB0] text-white text-center py-4 text-[1.35rem] font-normal tracking-wide shadow-sm">
                Verify your account
              </div>
              ...
              <div className="px-8 py-6 flex flex-col flex-1 bg-gray-50">
                {/* Google Logo */}
                <div className="text-[1.7rem] font-bold mb-6 tracking-tighter flex items-center">
                ...
                {/* Instruction Text */}
                <p className="text-[#202124] text-[15px] leading-[1.6] mb-12 font-medium">
                ...
                <div className="relative mt-2">
                  <div className="border-b-[1.5px] border-gray-300 pb-1.5">
                    <span className="text-gray-500 text-[15px]">Email or phone</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Dynamic Bottom Fade Overlay (Blends phone into background) */}
          <div className={`absolute bottom-0 left-[-20px] right-[-20px] h-[240px] z-20 pointer-events-none transition-colors duration-500 bg-gradient-to-t ${
            isColor ? 'from-[#050505] via-[#050505]/95' : 'from-slate-50 via-slate-50/95'
          } to-transparent`}></div>
        </div>

        {/* Divider */}
        <div className="w-full relative h-px mb-12 mt-4">
          <div className={`absolute inset-0 bg-gradient-to-r from-transparent ${
            isColor ? 'via-cyan-500/20' : 'via-blue-500/20'
          } to-transparent`}></div>
        </div>

        {/* List Header */}
        <div className="w-full max-w-xl text-center mb-8 relative z-30">
          <h2 className={`text-2xl md:text-3xl font-black tracking-wider uppercase mb-2 ${
            isColor ? 'text-white' : 'text-slate-900'
          }`}>
            Featured Tools
          </h2>
          <p className={`text-sm md:text-base font-bold uppercase tracking-widest ${
            isColor ? 'text-cyan-500/80' : 'text-blue-600/80'
          }`}>
            Tap to download
          </p>
        </div>
        ...
        <div className="w-full max-w-xl flex flex-col gap-4 mb-16 relative z-30">
          {tools.map((tool, idx) => (
            <Link 
              href={tool.href}
              key={idx} 
              className={`rounded-2xl p-4 flex items-center gap-5 cursor-pointer border backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
                isColor 
                  ? 'bg-[#0a0a0a]/80 border-slate-800 hover:border-cyan-500/30 hover:shadow-[0_8px_30px_rgba(6,182,212,0.1)]' 
                  : 'bg-white border-slate-200 hover:border-blue-500/30 hover:shadow-[0_8px_30px_rgba(37,99,235,0.1)]'
              }`}
            >
              {/* The comment that broke it is gone or moved here inside Link */}
      ...
      <div className="w-full py-8 flex justify-center mt-auto z-30">
        <Link 
          href="/tools" 
          className={`group flex items-center gap-2 text-xs font-bold tracking-[0.2em] transition-colors uppercase ${
            isColor ? 'text-slate-500 hover:text-cyan-400' : 'text-slate-400 hover:text-blue-600'
          }`}
        >
          See more
