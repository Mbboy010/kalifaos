...
export default function SearchPage() {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        {results.length > 0 ? (
          results.map((item) => (
            <Link
              key={item.id}
              href={activeTab === "windows" ? `/windows-tools/${item.id}` : item.route}
      ...
      {/* Disclaimer */}
      <div
        className="mt-10 p-4 rounded-lg text-center text-sm border border-gray-300 dark:border-gray-700"
        style={{ backgroundColor: isColor ? "#d7d7d719" : "#72727236" }}
      >
        <p>
          Use these tools responsibly and only on devices you legally own. Kalifa OS is not
          responsible for any misuse or device issues.
        </p>
      </div>
    </div>
  );
