import {
  Settings, Image, Calculator, Calendar, FileText, Mail, Music,
  Video, Globe, MessageSquare, Clock
} from 'lucide-react';
...
import {
  AndroidLogoCustom, SamsungLogoCustom, InfinixLogoCustom, TecnoLogoCustom
} from "../svg/Icons";
...
export default function SystemCon() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const updatedData = results.reduce((acc, { brand, items }) => {
        acc[brand] = items;
        return acc;
        ...
        {data.map(({ id, title, link }) => (
          ...
          >
            <div
              ...
              className="flex items-center hover:bg-gray-700 p-3 shadow rounded-lg transition-all duration-200"
            >
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="p-6 mb-7">
          <h1 className="text-3xl font-bold mb-2">System Applications</h1>
          <p>Access and manage your system applications</p>
        </div>

        <div className="w-screen mb-4 flex flex-col gap-2 min-h-screen">
          {hasData ? (
  ...
  <div className="w-screen flex flex-col gap-2 min-h-screen">
    {[...Array(2)].map((_, sectionIdx) => (
      <div key={sectionIdx} className="w-full max-w-5xl mx-auto flex flex-col mb-10">
        <div className="flex justify-center items-center text-center text-blue-500 mb-8">
          <h2 className="text-lg font-semibold coussstwo h-5 w-[50%] text-blue-600"></h2>
        </div>
        {[...Array(3)].map((_, idx) => (
          <a key={idx} className="px-4 py-1">
            <div style={{ backgroundColor: "#72727236" }} className="flex items-center p-2 shadow rounded-lg">
